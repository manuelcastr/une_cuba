...
def plot_daily_generation_and_demand(headers, dates, data, suptitle='', **kwargs):
    generation, demand, outage = data.T
    ...
    x = np.arange(n_days)
    axes.fill_between(x, np.zeros(n_days), generation, color='tab:green', edgecolor='k', lw=.8, zorder=100, label=headers[0])
    axes.plot(x, demand, color='tab:red', lw='3', zorder=200, label=headers[1])
    axes.fill_between(x, np.zeros(n_days), outage, color='tab:brown', edgecolor='k', lw=1.2, zorder=300, label=headers[2])

    x_labels = np.array([''] * len(dates), dtype=object)
    x_labels[::date_step] = np.array([d.strftime("%d/%m") for d in dates], dtype=object)[::date_step]
    axes.set_xticks(range(n_days), x_labels, rotation=-45, fontsize='x-small')
